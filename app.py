...
if run and ticker_input.strip():
    ...
    if hist is None:
        ...
        if dl:
            ...
            # Each day as an expander (data already pre-fetched)
            for d in reversed(dl):
                ...
                with st.expander(exp_label):
                    ...
                    if bars:
                        # Table with visual bar + gain/loss on 1K
                        html = (
                            f'<table class="atr-table"><thead><tr>'
                            f'<th>Ora</th><th>Min</th><th>Max</th><th>Var%</th><th style="min-width:70px"></th><th>{cur}1K</th><th>Vol</th>'
                            f'</tr></thead><tbody>')
                        for b in bars:
                            vp = b.get("var_pct")
                            # Visual bar
                            if vp is not None and vp != 0:
                                bar_color = "#00ff88" if vp > 0 else "#ff4757"
                                bar_w = min(abs(vp) * 15, 100)  # scale: 1% = 15px, max 100px
                                if vp > 0:
                                    bar_html = f'<div style="display:flex;justify-content:flex-start;"><div style="width:{bar_w:.0f}%;max-width:100%;height:6px;background:{bar_color};border-radius:3px;"></div></div>'
                                else:
                                    bar_html = f'<div style="display:flex;justify-content:flex-end;"><div style="width:{bar_w:.0f}%;max-width:100%;height:6px;background:{bar_color};border-radius:3px;"></div></div>'
                                # Gain/loss on 1000
                                gl = vp / 100 * 1000
                                gl_color = "#00ff88" if gl > 0 else "#ff4757"
                                gl_sign = "+" if gl > 0 else ""
                                gl_html = f'<span style="color:{gl_color}">{gl_sign}{gl:.1f}</span>'
                            else:
                            ...
                            html += (
                                ...
                                f'<td>{pct_badge(vp)}</td>'
                                f'<td style="padding:0 .2rem">{bar_html}</td>'
                                f'<td>{gl_html}</td>'
                                f'<td>{fmt_num(b["volume"],0)}</td></tr>')
                        html += '</tbody></table>'

                        # Cumulative gain/loss for the day
                        day_moves = [b.get("var_pct") for b in bars if b.get("var_pct") is not None]
                        if day_moves:
                            # Compound return
                            cumulative = 1.0
                            for m in day_moves:
                                cumulative *= (1 + m / 100)
                            day_return_pct = (cumulative - 1) * 100
                            day_gl = day_return_pct / 100 * 1000
                            gl_color = "#00ff88" if day_gl >= 0 else "#ff4757"
                            gl_sign = "+" if day_gl >= 0 else ""
                            html += (
                                ...
                                f'<div style="font-size:.6rem;color:var(--text-secondary);text-transform:uppercase;letter-spacing:.06em;">Risultato giornata su {cur}1.000 investiti</div>'
                                f'<div style="font-family:var(--font-mono);font-size:1.2rem;font-weight:600;color:{gl_color};margin-top:.2rem;">'
                                f'{gl_sign}{cur}{abs(day_gl):.2f} ({gl_sign}{day_return_pct:.2f}%)</div></div>')
                        ...
                        if moves:
                            ...
                            if avg_m >= 1.5: s_lbl = 'Molto agitata'
                            elif avg_m >= 0.5: s_lbl = 'Moderata'
                            else: s_lbl = 'Calma'

                            st.markdown(
                                f'<div style="font-size:.72rem;color:var(--text-secondary);margin-top:.5rem;line-height:1.6;">'
                                f'Osc. max: <b style="color:var(--text-primary)">{pct_badge(moves[max_idx])}</b> ore {max_bar["time"]} &nbsp;\u00b7&nbsp; '
                                f'Media: <b style="color:var(--text-primary)">{avg_m:.2f}%</b> &nbsp;\u00b7&nbsp; '
                                f'Barre &gt;1%: <b style="color:var(--text-primary)">{strong}/{len(moves)}</b><br>'
                                f'\u25b2<span style="color:var(--accent-green)">{up}</span> / '
                                f'\u25bc<span style="color:var(--accent-red)">{down}</span> &nbsp;\u00b7&nbsp; '
                                f'{d_lbl} &nbsp;\u00b7&nbsp; {s_lbl}</div>',
                                unsafe_allow_html=True)
                    else:
                        st.markdown(
