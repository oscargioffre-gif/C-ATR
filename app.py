...
def is_isin(t): return bool(ISIN_RE.match(t.upper()))

@st.cache_data(ttl=3600, show_spinner=False)
def resolve_isin(isin):
    resp = safe_fetch(f"{YAHOO_SEARCH}?q={isin.upper()}&quotesCount=5&newsCount=0")
    if not resp: return None
    try:
        for q in resp.json().get("quotes", []):
            if q.get("quoteType") == "EQUITY": return q.get("symbol")
...
def resolve_input(inp):
    t = inp.strip().upper()
    if not t: return None, "Inserisci un ticker o ISIN."
    if is_isin(t):
        tk = resolve_isin(t)
        return (tk, f"ISIN {t} \u2192 {tk}") if tk else (None, f"ISIN {t} non trovato.")
    return t, None
...
# ─── FETCH INTRADAY 30-MIN FOR A SPECIFIC DAY ────────────────────────────────

@st.cache_data(ttl=600, show_spinner=False)
def fetch_intraday(ticker, target_date):
    """Fetch 30-min bars for a specific date (regular session only)."""
    ic = None
    for rng in ["5d","1mo","3mo"]:
    ...
    try:
        ...
        # Get reg bounds
        meta = ic.get("meta", {})
        rs, re_ = get_reg_bounds(meta, its)

        bars = []
        for i, t in enumerate(its):
            d = datetime.utcfromtimestamp(t)
            if d.strftime("%Y-%m-%d") != target_date: continue
            # Skip if outside regular session hours (approximate using time)
...
<b style="color:#00d4ff;">Range%</b> — (Max-Min)/Min \u00d7 100 del giorno. L\u2019oscillazione reale di quella giornata.

<b style="color:#00d4ff;">Var%</b> — Variazione rispetto alla chiusura del giorno precedente (giornaliero) o alla mezz\u2019ora precedente (intraday).

<b style="color:#00d4ff;">Tabella cliccabile</b> — Seleziona un giorno dal menu per vedere il dettaglio ogni 30 minuti di quella seduta.
...
if run and ticker_input.strip():
    ...
    if "." not in ticker:
        ...
        if rt is None:
            ...
            if rt: ticker, cur, mkt = ticker + ".MI", "\u20ac", "Borsa Italiana"
    else:
        rt = fetch_realtime(ticker)

    if rt is None:
        st.markdown(f'<div class="card" style="border-color:var(--accent-red);"><div class="card-title">\u26a0\ufe0f ERRORE</div><div class="card-sub">Impossibile recuperare dati per <b>{raw}</b>.</div></div>', unsafe_allow_html=True)
        st.stop()
    ...
    if hist is None:
        ...
        # Daily history table — all in %
        dl = [d for d in hist["daily"] if d.get("atr_pct") is not None][-22:]
        if dl:
            html = ('<div class="card"><div class="card-title">\U0001f4c5 Storico \u00b7 Tutto in %</div>'
            ...
            # ── Day selector for intraday detail ──
            available_dates = [d["date"] for d in reversed(dl)]
            selected_day = st.selectbox(
                "\U0001f50d Seleziona un giorno per il dettaglio 30min",
                options=available_dates,
                index=0,
            )

            if selected_day:
                with st.spinner(f"Caricamento {selected_day}..."):
                    bars = fetch_intraday(ticker, selected_day)

                if bars:
                    # Intraday table
                    html = (
                        f'<div class="card"><div class="card-title">\u23f1 Dettaglio 30min \u00b7 {selected_day}</div>'
                    ...
                    for b in bars:
                        html += (
                            ...
                            f'<td style="color:var(--accent-amber)">{b["range_pct"]:.2f}%</td>'
                            f'<td>{pct_badge(b.get("var_pct"))}</td>'
                            f'<td>{fmt_num(b["volume"],0)}</td></tr>')
                    html += '</tbody></table></div>'
                    st.markdown(html, unsafe_allow_html=True)

                    # Session summary
                    moves = [b["var_pct"] for b in bars if b.get("var_pct") is not None]
                    if moves:
                        abs_m = [abs(m) for m in moves]
                        max_m = max(abs_m)
                        max_idx = abs_m.index(max_m)
                        max_bar = [b for b in bars if b.get("var_pct") is not None][max_idx]
                        avg_m = sum(abs_m) / len(abs_m)
                        strong = sum(1 for m in abs_m if m >= 1.0)
                        medium = sum(1 for m in abs_m if m >= 0.5)
                        up = sum(1 for m in moves if m > 0)
                        down = sum(1 for m in moves if m < 0)

                        if up > down * 1.5: d_lbl = '<span style="color:var(--accent-green)">\u25b2 Rialzista</span>'
                        elif down > up * 1.5: d_lbl = '<span style="color:var(--accent-red)">\u25bc Ribassista</span>'
                        else: d_lbl = '<span style="color:var(--accent-amber)">\u25c6 Laterale</span>'

                        if avg_m >= 1.5: s_lbl = '<span style="color:var(--accent-red)">Molto agitata</span>'
                        elif avg_m >= 0.5: s_lbl = '<span style="color:var(--accent-amber)">Moderata</span>'
                        else: s_lbl = '<span style="color:var(--accent-green)">Calma</span>'

                        st.markdown(
                            f'<div class="card"><div class="card-title">\U0001f4cb Riepilogo {selected_day}</div>'
                            f'<div class="metric-row" style="margin-bottom:.4rem">'
                            f'<div class="metric-box"><div class="metric-label">Osc. Max</div>'
                            f'<div class="metric-val">{pct_badge(moves[max_idx])}</div>'
                            f'<div style="font-size:.6rem;color:var(--text-secondary);margin-top:.15rem;">ore {max_bar["time"]}</div></div>'
                            f'<div class="metric-box"><div class="metric-label">Osc. Media</div>'
                            f'<div class="metric-val">{avg_m:.2f}%</div></div></div>'
                            f'<div class="metric-row" style="margin-bottom:.4rem">'
                            f'<div class="metric-box"><div class="metric-label">Barre &gt;1%</div>'
                            f'<div class="metric-val">{strong}<span style="font-size:.7rem;color:var(--text-secondary);">/{len(moves)}</span></div></div>'
                            f'<div class="metric-box"><div class="metric-label">Barre &gt;0.5%</div>'
                            f'<div class="metric-val">{medium}<span style="font-size:.7rem;color:var(--text-secondary);">/{len(moves)}</span></div></div>'
                            ...
                            f'<span style="color:var(--accent-red)">{down}</span></div></div></div>'
                            f'<div style="text-align:center;margin-top:.3rem;">'
                            f'<div style="font-size:.7rem;color:var(--text-secondary);">{d_lbl} &nbsp;\u00b7&nbsp; {s_lbl}</div>'
                            f'</div></div>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="card" style="border-color:var(--accent-amber);"><div class="card-title">\u26a0\ufe0f</div><div class="card-sub">Dati intraday non disponibili per {selected_day}.</div></div>', unsafe_allow_html=True)
