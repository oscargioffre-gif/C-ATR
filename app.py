...
if run and ticker_input.strip():
    ...
    # Volume with Buy/Sell pressure — split by session
    st.markdown(
        f'<div class="card">'
        f'<div class="card-title">\U0001f4ca Volume Sessione Regolare</div>'
        f'<div class="metric-row" style="margin-bottom:.4rem">'
        f'<div class="metric-box"><div class="metric-label">Vol. Regolare</div><div class="metric-val">{fmt_num(rt["reg_total"],1)}</div></div>'
        f'<div class="metric-box"><div class="metric-label">Vol. Totale (incl. esteso)</div><div class="metric-val">{fmt_num(rt["total_volume"],1)}</div></div></div>'
