...
# ─── PAGE CONFIG ──────────────────────────────────────────────────────────────
# Load icon from static/ — PIL is bundled with Streamlit
...
st.markdown(
    ...
    '<link rel="shortcut icon" type="image/png" href="/app/static/icon.png">',
...
def fetch_realtime(ticker):
    ...
    try:
        ...
        # FIX: currentTradingPeriod may refer to the NEXT trading day when queried
        # outside regular hours (e.g. from Europe at night). All bars would fall
        # before reg_start, wrongly classified as "pre-market".
        # Solution: if bars are from a previous date, shift reg_start/reg_end back.
        shift_seconds = 0
        if timestamps and reg_start > 0:
            last_bar_ts = timestamps[-1]
            if last_bar_ts < reg_start:
                from datetime import timezone
                reg_dt = datetime.fromtimestamp(reg_start, tz=timezone.utc).date()
                bar_dt = datetime.fromtimestamp(last_bar_ts, tz=timezone.utc).date()
                day_diff = (reg_dt - bar_dt).days
                if day_diff > 0:
                    shift_seconds = day_diff * 86400
                    reg_start -= shift_seconds
                    reg_end -= shift_seconds
        ...
        for i in range(len(timestamps)):
            ...
            if h is None or lo is None:
                continue
            if ts < reg_start:
                pre_highs.append(h); pre_lows.append(lo)
            elif ts >= reg_end:
                ah_highs.append(h); ah_lows.append(lo)
            else:
                reg_highs.append(h); reg_lows.append(lo)
        ...
        return {
            ...
            "pre_high": round(max(pre_highs), 4) if pre_highs else None,
            "pre_low": round(min(pre_lows), 4) if pre_lows else None,
            "ah_high": round(max(ah_highs), 4) if ah_highs else None,
            "ah_low": round(min(ah_lows), 4) if ah_lows else None,
            "name": name,
        }
    except Exception:
        return None
...
<b>Range</b> \u2014 Differenza tra Max e Min della candela (ampiezza dello spread).<br>
<b>Var%</b> \u2014 Quanto si \u00e8 spostato il prezzo dalla mezz\u2019ora precedente. \u00c8 il dato chiave per capire le oscillazioni.<br>
\u2022 Barre con <b>Var% alta + Volume alto</b> = momento di forte attivit\u00e0.

<h3>\U0001f4cb Riepilogo Sessione</h3>
<b>Osc. Max</b> \u2014 Il pi\u00f9 grande spostamento in una singola mezz\u2019ora (con orario).<br>
<b>Osc. Media</b> \u2014 Media degli spostamenti (in valore assoluto). Indica se la giornata \u00e8 stata agitata o calma.<br>
<b>Barre &gt;1% / &gt;0.5%</b> \u2014 Quante volte il titolo ha oscillato sopra soglia.<br>
<b>\u25b2/\u25bc</b> \u2014 Barre in salita vs in discesa.<br>
\u2022 <span class="signal-buy">Calma</span> = osc. media &lt;0.5% \u2022 <span class="signal-warn">Moderata</span> = 0.5-1.5% \u2022 <span class="signal-sell">Molto agitata</span> = &gt;1.5%.
...
# ─── HEADER ───────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="app-header"><h1>\U0001f7e2 V2</h1>'
...
if run and ticker_input.strip():
    ...
    if rt["reg_high"] and rt["reg_low"]:
        ...
        st.markdown(
            f'<div class="card">'
            f'<div class="card-title">\U0001f4ca Range Sessione Regolare</div>'
            f'<div class="metric-row" style="margin-bottom:0">'
            ...
            f'<div class="metric-box"><div class="metric-label">Spread</div><div class="metric-val">{cur}{sp:,.2f}</div></div>'
            f'</div></div>', unsafe_allow_html=True)

    # Pre-Market range
    if rt["pre_high"] and rt["pre_low"]:
        sp_pre = rt["pre_high"] - rt["pre_low"]
        st.markdown(
            f'<div class="card">'
            f'<div class="card-title">\U0001f305 Range Pre-Market</div>'
            f'<div class="metric-row" style="margin-bottom:0">'
            f'<div class="metric-box"><div class="metric-label">Min</div><div class="metric-val" style="color:var(--accent-red)">{cur}{rt["pre_low"]:,.2f}</div></div>'
            f'<div class="metric-box"><div class="metric-label">Max</div><div class="metric-val" style="color:var(--accent-green)">{cur}{rt["pre_high"]:,.2f}</div></div>'
            f'<div class="metric-box"><div class="metric-label">Spread</div><div class="metric-val">{cur}{sp_pre:,.2f}</div></div>'
            f'</div></div>', unsafe_allow_html=True)

    # After-Hours range
    if rt["ah_high"] and rt["ah_low"]:
        sp_ah = rt["ah_high"] - rt["ah_low"]
        st.markdown(
            f'<div class="card">'
            f'<div class="card-title">\U0001f319 Range After-Hours</div>'
            f'<div class="metric-row" style="margin-bottom:0">'
            f'<div class="metric-box"><div class="metric-label">Min</div><div class="metric-val" style="color:var(--accent-red)">{cur}{rt["ah_low"]:,.2f}</div></div>'
            f'<div class="metric-box"><div class="metric-label">Max</div><div class="metric-val" style="color:var(--accent-green)">{cur}{rt["ah_high"]:,.2f}</div></div>'
            f'<div class="metric-box"><div class="metric-label">Spread</div><div class="metric-val">{cur}{sp_ah:,.2f}</div></div>'
            f'</div></div>', unsafe_allow_html=True)
    ...
    if show_atr:
        ...
        if atr is None:
            ...
            if atr["intraday_30min"]:
                bars = atr["intraday_30min"]
                html = f'<div class="card"><div class="card-title">\u23f1 Intraday 30min \u00b7 {atr["last_trading_day"] or ""}</div><table class="atr-table"><thead><tr><th>Ora</th><th>Min</th><th>Max</th><th>Range</th><th>Var%</th><th>Vol</th></tr></thead><tbody>'
                for s in bars:
                    html += f'<tr><td>{s["time"]}</td><td style="color:var(--accent-red)">{cur}{s["low"]:,.2f}</td><td style="color:var(--accent-green)">{cur}{s["high"]:,.2f}</td><td>{cur}{s["range"]:,.2f}</td><td>{pct_badge(s.get("pct_change"))}</td><td>{fmt_num(s["volume"],0)}</td></tr>'
                html += '</tbody></table></div>'
                st.markdown(html, unsafe_allow_html=True)

                # ── Session summary ───────────────────────────────────────
                moves = [b["pct_change"] for b in bars if b.get("pct_change") is not None]
                if moves:
                    abs_moves = [abs(m) for m in moves]
                    max_move = max(abs_moves)
                    max_move_idx = abs_moves.index(max_move)
                    max_move_bar = [b for b in bars if b.get("pct_change") is not None][max_move_idx]
                    max_move_sign = moves[max_move_idx]

                    # Count significant moves (>1% and >0.5%)
                    strong_bars = sum(1 for m in abs_moves if m >= 1.0)
                    medium_bars = sum(1 for m in abs_moves if m >= 0.5)
                    up_bars = sum(1 for m in moves if m > 0)
                    down_bars = sum(1 for m in moves if m < 0)
                    flat_bars = sum(1 for m in moves if m == 0)
                    avg_move = sum(abs_moves) / len(abs_moves)

                    # Direction label
                    if up_bars > down_bars * 1.5:
                        dir_label = '<span style="color:var(--accent-green)">\u25b2 Prevalenza rialzista</span>'
                    elif down_bars > up_bars * 1.5:
                        dir_label = '<span style="color:var(--accent-red)">\u25bc Prevalenza ribassista</span>'
                    else:
                        dir_label = '<span style="color:var(--accent-amber)">\u25c6 Laterale / equilibrio</span>'

                    # Volatility label
                    if avg_move >= 1.5:
                        vol_label = '<span style="color:var(--accent-red)">Molto agitata</span>'
                    elif avg_move >= 0.5:
                        vol_label = '<span style="color:var(--accent-amber)">Moderata</span>'
                    else:
                        vol_label = '<span style="color:var(--accent-green)">Calma</span>'

                    max_badge = pct_badge(max_move_sign)

                    st.markdown(
                        f'<div class="card">'
                        f'<div class="card-title">\U0001f4cb Riepilogo Sessione</div>'
                        f'<div class="metric-row" style="margin-bottom:.4rem">'
                        f'<div class="metric-box"><div class="metric-label">Osc. Max (singola barra)</div>'
                        f'<div class="metric-val">{max_badge}</div>'
                        f'<div style="font-size:.6rem;color:var(--text-secondary);margin-top:.15rem;">ore {max_move_bar["time"]}</div></div>'
                        f'<div class="metric-box"><div class="metric-label">Osc. Media</div>'
                        f'<div class="metric-val">{avg_move:.2f}%</div></div>'
                        f'</div>'
                        f'<div class="metric-row" style="margin-bottom:.4rem">'
                        f'<div class="metric-box"><div class="metric-label">Barre &gt;1%</div>'
                        f'<div class="metric-val">{strong_bars}<span style="font-size:.7rem;color:var(--text-secondary);">/{len(moves)}</span></div></div>'
                        f'<div class="metric-box"><div class="metric-label">Barre &gt;0.5%</div>'
                        f'<div class="metric-val">{medium_bars}<span style="font-size:.7rem;color:var(--text-secondary);">/{len(moves)}</span></div></div>'
                        f'<div class="metric-box"><div class="metric-label">\u25b2/\u25bc</div>'
                        f'<div class="metric-val" style="font-size:.85rem;">'
                        f'<span style="color:var(--accent-green)">{up_bars}</span>'
                        f'<span style="color:var(--text-secondary)"> / </span>'
                        f'<span style="color:var(--accent-red)">{down_bars}</span></div></div>'
                        f'</div>'
                        f'<div style="text-align:center;margin-top:.3rem;">'
                        f'<div style="font-size:.7rem;color:var(--text-secondary);">Direzione: {dir_label} &nbsp;\u00b7&nbsp; Giornata: {vol_label}</div>'
                        f'</div></div>', unsafe_allow_html=True)

            if atr["daily"]:
                dl = [d for d in atr["daily"] if d.get("atr") is not None][-22:]
                html = '<div class="card"><div class="card-title">\U0001f4c5 Storico Mese</div><table class="atr-table"><thead><tr><th>Data</th><th>Min</th><th>Max</th><th>Range</th><th>Var%</th><th>Vol</th></tr></thead><tbody>'
                for d in reversed(dl):
                    day_range = d["high"] - d["low"]
                    html += f'<tr><td style="font-size:.7rem">{d["date"]}</td><td style="color:var(--accent-red)">{cur}{d["low"]:,.2f}</td><td style="color:var(--accent-green)">{cur}{d["high"]:,.2f}</td><td style="color:var(--accent-amber)">{cur}{day_range:,.2f}</td><td>{pct_badge(d["pct_change"])}</td><td>{fmt_num(d.get("volume",0),0)}</td></tr>'
                html += '</tbody></table></div>'
                st.markdown(html, unsafe_allow_html=True)
