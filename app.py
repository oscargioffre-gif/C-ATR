            ...
            if its:
                ...
                if last_td:
                    ...
                    for i, t in enumerate(its):
                        ...
                        intraday.append({
                            "time": d.strftime("%H:%M"),
                            "open": round(ov, 4) if ov else None,
                            "close": round(cv, 4) if cv else None,
                            "high": round(hv, 4),
                            "low": round(lv, 4),
                            "range": round(hv - lv, 4),
                            "volume": int(vv),
                        })
                    # Compute % change vs previous bar close
                    for idx, bar in enumerate(intraday):
                        if idx == 0 or intraday[idx - 1].get("close") is None or bar.get("close") is None:
                            bar["pct_change"] = None
                        else:
                            prev_c = intraday[idx - 1]["close"]
                            bar["pct_change"] = round((bar["close"] - prev_c) / prev_c * 100, 2) if prev_c else None
