...
def fetch_realtime(ticker):
    ...
    try:
        ...
        for i in range(len(timestamps)):
            ...
            mid = (h + lo) / 2
            is_extended = (timestamps[i] < reg_start or timestamps[i] >= reg_end)

            # Determine direction: compare midpoint to previous bar's midpoint
            if prev_mid is not None:
                ...
                # First bar: use open vs close of bar itself
...
if run and ticker_input.strip():
    ...
    # Volume with Buy/Sell pressure
    buy_pct = rt["buy_volume"] / rt["total_volume"] * 100 if rt["total_volume"] > 0 else 50
    buy_color = "var(--accent-green)" if buy_pct >= 55 else ("var(--accent-red)" if buy_pct < 45 else "var(--text-primary)")
    st.markdown(
        f'<div class="card">'
        f'<div class="card-title">\U0001f4ca Volume Sessione Regolare + Estesa</div>'
        f'<div class="metric-row" style="margin-bottom:.4rem">'
        f'<div class="metric-box"><div class="metric-label">Volume Totale</div><div class="metric-val">{fmt_num(rt["total_volume"],1)}</div></div>'
        f'<div class="metric-box"><div class="metric-label">Vol. Esteso</div><div class="metric-val">{fmt_num(rt["ext_total"],1)}</div></div></div>'
        f'{pressure_bar_html(rt["buy_volume"], rt["sell_volume"])}'
        f'<div class="card-note">\u26a0\ufe0f Stima basata sulla direzione del prezzo (metodo midpoint). Non \u00e8 un dato certo.</div>'
        f'</div>', unsafe_allow_html=True)

    # Extended hours pressure (only if there's volume)
    if rt["ext_total"] > 0:
        st.markdown(
            f'<div class="card">'
            f'<div class="card-title">\U0001f319 Pressione Pre-Market / After-Hours</div>'
            f'{pressure_bar_html(rt["ext_buy"], rt["ext_sell"])}'
            f'</div>', unsafe_allow_html=True)
    ...
    if show_atr:
        ...
        if atr is None:
            ...
            if atr["daily"]:
                ...
                html = '<div class="card"><div class="card-title">\U0001f4c5 Storico Mese</div><table class="atr-table"><thead><tr><th>Data</th><th>Min</th><th>Max</th><th>ATR</th><th>Var%</th></tr></thead><tbody>'
                for d in reversed(dl):
                    html += f'<tr><td style="font-size:.7rem">{d["date"]}</td><td style="color:var(--accent-red)">{cur}{d["low"]:,.2f}</td><td style="color:var(--accent-green)">{cur}{d["high"]:,.2f}</td><td style="color:var(--accent-amber)">{cur}{d["atr"]:,.2f}</td><td>{pct_badge(d["pct_change"])}</td></tr>'
                html += '</tbody></table></div>'
                st.markdown(html, unsafe_allow_html=True)
