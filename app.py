...
def fetch_analyst_data(ticker):
    """Fetch analyst data. Fast: tries all-at-once first, then fills gaps."""
    result = {
        "target_mean": None, "target_high": None, "target_low": None,
        "current": None, "rec_key": "", "rec_mean": None,
        "num_analysts": 0, "rec_detail": {},
    ...
    # Strategy 1: all modules at once (covers 90% of cases)
    r = _try_summary(ticker, "financialData,recommendationTrend,calendarEvents,earningsHistory,earningsTrend,price")
    if r:
        result = _fill(result, _parse_summary(r))

    # Early exit if we have the key data
    if result["target_mean"] and result["past_earnings"]:
        return result

    # Strategy 2: fill gaps with individual modules (only what's missing)
    if not result["target_mean"]:
        r2 = _try_summary(ticker, "financialData,price")
        if r2:
            result = _fill(result, _parse_summary(r2))
    if not result["past_earnings"] and not result["next_earnings"]:
        r3 = _try_summary(ticker, "calendarEvents,earningsHistory")
        if r3:
            result = _fill(result, _parse_summary(r3))

    # Strategy 3: upgradeDowngrade for sentiment fallback
    ...
    if st.session_state.show_trim:
        ...
        if analyst_trim:
            ...
            if a.get("past_earnings"):
                hist_for_impact = hist  # already fetched in parallel
