...
def safe_fetch(url, retries=1, timeout=6):
...
def _try_summary(ticker, modules_str):
    """Try fetching quoteSummary. Fast: stops at first success."""
    for base in SUMMARY_URLS:
        resp = safe_fetch(f"{base.format(ticker=ticker)}?modules={modules_str}", retries=1, timeout=6)
        if not resp:
            continue
        try:
            data = resp.json()
...
def fetch_analyst_data(ticker):
    """Fetch analyst target price + recommendations only. Fast."""
    result = {
        "target_mean": None, "target_high": None, "target_low": None,
        "current": None, "rec_key": "", "rec_mean": None,
        "num_analysts": 0, "rec_detail": {},
    ...
    # Strategy 1: financialData + recommendationTrend + price
    r = _try_summary(ticker, "financialData,recommendationTrend,price")
    if r:
        result = _fill(result, _parse_summary(r))

    # Early exit if we have the key data
    if result["target_mean"] and result["rec_key"]:
        return result

    # Strategy 2: individual modules
    if not result["target_mean"]:
        r2 = _try_summary(ticker, "financialData,price")
        if r2:
            result = _fill(result, _parse_summary(r2))

    # Strategy 3: upgradeDowngrade for recommendation fallback
    if not result["rec_key"]:
        r3 = _try_summary(ticker, "upgradeDowngradeHistory")
        if r3:
            hist = r3.get("upgradeDowngradeHistory", {}).get("history", [])
            if hist:
                g = hist[0].get("toGrade", "").lower()
                if any(w in g for w in ("buy", "outperform", "overweight")):
                    result["rec_key"] = "buy"
                elif any(w in g for w in ("sell", "underperform", "underweight")):
                    result["rec_key"] = "sell"
                else:
                    result["rec_key"] = "hold"

    has_data = result["target_mean"] or result["rec_key"]
...
def fetch_history(ticker):
    ...
    try:
        ...
        vols = q.get("volume", [])
        gmt_offset = dc.get("meta", {}).get("gmtoffset", 0)
        ...
        for i in range(len(ts)):
            ...
            local_ts = ts[i] + gmt_offset
            rows.append({
                "date": datetime.utcfromtimestamp(local_ts).strftime("%Y-%m-%d"),
...
def fetch_all_intraday(ticker):
    """Fetch intraday bars for ALL available days, return dict of date->bars."""
    ic = None
    # Try 30m first, then 15m, then 1h — different tickers support different intervals
    for intv in ["30m", "15m", "1h"]:
        for rng in ["1mo", "3mo"]:
            ic = yahoo_chart(ticker, interval=intv, range_=rng)
            if ic and ic.get("timestamp") and len(ic["timestamp"]) > 5:
                break
            ic = None
        if ic:
            break
    ...
    try:
        ...
        # Use exchange timezone offset from meta if available
        meta = ic.get("meta", {})
        gmt_offset = meta.get("gmtoffset", 0)  # seconds offset from UTC

        # Group bars by date (using exchange local date)
        all_bars = {}
        for i, t in enumerate(its):
            # Convert to exchange local date
            local_ts = t + gmt_offset
            ...
            all_bars[date_str].append({
                "time": time_str, "high": round(hv,4), "low": round(lv,4),
...
if st.session_state.active_ticker:
    ...
    with st.spinner("Caricamento dati..."):
        with ThreadPoolExecutor(max_workers=4) as pool:
            f_analyst = pool.submit(fetch_analyst_data, ticker)
            f_vol = pool.submit(fetch_volume_stats, ticker)
            f_hist = pool.submit(fetch_history, ticker)
            f_intra = pool.submit(fetch_all_intraday, ticker)
        analyst = f_analyst.result()
        vs = f_vol.result()
        hist = f_hist.result()
        all_intraday = f_intra.result()

    # Compute RSI from history data (no extra API call)
    rsi_val = None
    if hist and hist.get("daily"):
        rsi_closes = [d["close"] for d in hist["daily"] if d.get("close")]
    ...
    # ── Volume vs 3M (data already fetched in parallel) ──
    if vs:
    ...
    # ── ATR% + History (data already fetched in parallel) ──
    if hist is None:
        ...
        # Daily history — last 30 trading days only
        dl = [d for d in hist["daily"] if d.get("atr_pct") is not None][-30:]
        if dl:
            # Pre-fetched intraday data (already loaded in parallel)

            # Table header
            st.markdown(
                '<div class="card"><div class="card-title">\U0001f4c5 Storico 30 giorni \u00b7 Tocca una data per il dettaglio</div>'
            ...
            # Expanders only for days with intraday data
            for d in reversed(dl):
                ...
                with st.expander(exp_label):
                    ...
                    for b in bars:
                        ...
                        if rp > 0:
                            bar_color = "#ff4757" if rp >= 2 else ("#ffb340" if rp >= 1 else "#00ff88")
                            bar_w = min(rp * 12, 100)
                            bar_html = f'<div style="width:{bar_w:.0f}%;height:6px;background:{bar_color};border-radius:3px;"></div>'
                            gl = rp / 100 * 1000
                            gl_html = f'<span style="color:#ff4757">-{gl:.1f}</span>' if dire == "down" else f'<span style="color:#00ff88">+{gl:.1f}</span>'
                        else:
                    ...
                    if ranges:
                        ...
                        html += (
                            ...
                            f'Oscillazione max in 30min su {cur}1.000</div>'
                            ...
                            f'Media: +{cur}{avg_gl:.2f} ({avg_range:.2f}%)</div></div>')
                    ...
                    st.markdown(
                        ...
                        f'Barre &gt;1%: <b style="color:var(--text-primary)">{strong}/{len(ranges)}</b> &nbsp;\u00b7&nbsp; '
                        f'{d_lbl} &nbsp;\u00b7&nbsp; {s_lbl}</div>',
                        unsafe_allow_html=True)
