            ...
            if its:
                ...
                if last_td:
                    ...
                    for idx,bar in enumerate(intraday):
                        ...
                        # Range% = range as percentage of midpoint price
                        bar["range_pct"] = round(bar["range"]/bar["mid"]*100,2) if bar["mid"] else None
...
<b>R%</b> \u2014 Range della candela in percentuale del prezzo. Utile per confrontare titoli di prezzo diverso.<br>
<b>Var%</b> \u2014 Scostamento rispetto alla candela precedente (quanto si \u00e8 spostato il prezzo da una mezz\u2019ora all\u2019altra).<br>
    ...
    if show_atr:
        ...
        if atr is None:
            ...
            if atr["intraday_30min"]:
                ...
                html = f'<div class="card"><div class="card-title">\u23f1 Intraday 30min \u00b7 {atr["last_trading_day"] or ""}</div><table class="atr-table"><thead><tr><th>Ora</th><th>Min</th><th>Max</th><th>Range</th><th>R%</th><th>Var%</th><th>Vol</th></tr></thead><tbody>'
                for s in bars:
                    rp = f'{s["range_pct"]:.1f}%' if s.get("range_pct") is not None else '\u2014'
                    html += f'<tr><td>{s["time"]}</td><td style="color:var(--accent-red)">{cur}{s["low"]:,.2f}</td><td style="color:var(--accent-green)">{cur}{s["high"]:,.2f}</td><td>{cur}{s["range"]:,.2f}</td><td>{rp}</td><td>{pct_badge(s.get("pct_change"))}</td><td>{fmt_num(s["volume"],0)}</td></tr>'
