                ...
                with st.expander(exp_label):
                    ...
                    if bars:
                        ...
                        html = (
                            ...
                            f'<th>Ora</th><th>Min</th><th>Max</th><th>Range%</th><th style="min-width:70px"></th><th>{cur}1K</th><th>Vol</th>'
                            f'</tr></thead><tbody>')
                        for b in bars:
                            rp = b.get("range_pct", 0)
                            # Visual bar — always green, proportional to range
                            if rp > 0:
                                if rp >= 2:
                                    bar_color = "#ff4757"   # high volatility = red
                                elif rp >= 1:
                                    bar_color = "#ffb340"   # moderate = amber
                                else:
                                    bar_color = "#00ff88"   # calm = green
                                bar_w = min(rp * 12, 100)
                                bar_html = f'<div style="width:{bar_w:.0f}%;height:6px;background:{bar_color};border-radius:3px;"></div>'
                                # $1K = max potential gain in this 30min
                                gl = rp / 100 * 1000
                        ...
                        # Session direction summary using var_pct (midpoint change)
