...
# Icon + Manifest paths via enableStaticServing=true
ICON_PATH = "/app/static/icon.png"
MANIFEST_PATH = "/app/static/manifest.json"

# ─── CSS + PWA ICON INJECTION ─────────────────────────────────────────────────
st.markdown(f"""
<script>
(function() {{
    var base = window.location.origin;
    var iconUrl = base + "{ICON_PATH}";
    var manifestUrl = base + "{MANIFEST_PATH}";

    function injectPWA() {{
        // 1. Manifest for Android "Add to Home Screen"
        ...
        // 2. Apple touch icon for iOS "Add to Home Screen"
        ...
        // 3. Override Streamlit's favicon
        document.querySelectorAll('link[rel="shortcut icon"], link[rel="icon"]:not([sizes="512x512"])').forEach(function(el) {{
            el.href = iconUrl;
            el.type = 'image/png';
        }});
    ...
    injectPWA();
    // Re-run on Streamlit re-renders (it modifies <head>)
    new MutationObserver(function(mutations) {{
        for (var i = 0; i < mutations.length; i++) {{
            for (var j = 0; j < mutations[i].addedNodes.length; j++) {{
                var node = mutations[i].addedNodes[j];
                if (node.tagName === 'LINK' && node.rel === 'shortcut icon') {{
                    node.href = iconUrl;
                    node.type = 'image/png';
                }}
            }}
        ...
        document.title = 'V2';
        // Re-add manifest/apple-touch-icon if removed
        if (!document.querySelector('link[rel="manifest"]')) {{
            var m = document.createElement('link');
            m.rel = 'manifest';
            m.href = manifestUrl;
