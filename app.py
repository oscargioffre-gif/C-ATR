...
def fetch_analyst_data(ticker):
    """Fetch analyst target price, recommendations, earnings dates."""
    modules = "financialData,recommendationTrend,calendarEvents,earningsHistory,earningsTrend"
    for base in SUMMARY_URLS:
        resp = safe_fetch(f"{base.format(ticker=ticker)}?modules={modules}")
        if not resp:
            continue
        try:
            data = resp.json()
            ...
            et = r.get("earningsTrend", {}).get("trend", [])
            ...
            current = fd.get("currentPrice", {}).get("raw")
            rec_key = fd.get("recommendationKey", "")  # buy, hold, sell, etc.
            rec_mean = fd.get("recommendationMean", {}).get("raw")  # 1=strongBuy, 5=sell
            num_analysts = fd.get("numberOfAnalystOpinions", {}).get("raw", 0)

            # Recommendation trend (current period)
            rec_detail = {}
            if rt_:
                curr = rt_[0]  # most recent period
                rec_detail = {
                    "strongBuy": curr.get("strongBuy", 0),
                    "buy": curr.get("buy", 0),
                    "hold": curr.get("hold", 0),
                    "sell": curr.get("sell", 0),
                    "strongSell": curr.get("strongSell", 0),
                }
            ...
            if ed:
                ...
                if ts:
                    next_earnings = datetime.utcfromtimestamp(ts).strftime("%Y-%m-%d")

            # Past earnings dates
            past_earnings = []
            for h in eh:
                qd = h.get("quarter", {}).get("fmt")
                ...
                if ed_raw:
                    past_earnings.append({
                        ...
                        "surprise_pct": round(surprise_pct * 100, 1) if surprise_pct else None,
                    })

            # Earnings trend / sentiment
            earnings_sentiment = None
            if et:
                for trend in et:
                    if trend.get("period") == "+1":  # next quarter
                        growth = trend.get("growth", {}).get("raw")
                        if growth is not None:
                            earnings_sentiment = round(growth * 100, 1)
                        break

            return {
                "target_mean": target_mean, "target_high": target_high,
                "target_low": target_low, "current": current,
                "rec_key": rec_key, "rec_mean": rec_mean,
                "num_analysts": num_analysts, "rec_detail": rec_detail,
                "next_earnings": next_earnings, "past_earnings": past_earnings,
...
if run and ticker_input.strip():
    ...
    with col_trim:
        ...
        trim_btn = st.button("\U0001f4c5 TRIM", use_container_width=True, help="Trimestrali")
    ...
    if trim_btn and analyst:
        ...
        if a.get("earnings_sentiment") is not None:
            ...
            if es > 5:
                es_lbl, es_color = "POSITIVO", "#00ff88"
                es_icon = "\u25b2"
            elif es < -5:
                es_lbl, es_color = "NEGATIVO", "#ff4757"
                es_icon = "\u25bc"
            else:
                es_lbl, es_color = "NEUTRO", "#ffb340"
                es_icon = "\u25c6"
            st.markdown(
        ...
        if a.get("past_earnings"):
            # Fetch full daily history for impact analysis
